...
/**
 * Initialize a 't_philo' structure with parameters derived from the
 * command-line arguments and monitor state. Sets fork pointers, timing
...
void	philo_print(t_philo *p, enum e_state state)
{
	...
	printf("%d %d %s\n", timestamp, id, state_names[state]);
}
